...
typedef struct
{
    ...
    int num_of_blocks; // Similar to last block id, count instead here
} HP_info;

/* Η δομή HP_Block_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού*/
typedef struct 
{
    int entries_num; // Pointer to next block would not work all the times, cause when block gets moved to disk address will be no valid anymore
} HP_Block_info;
